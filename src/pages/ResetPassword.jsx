...
const ResetPassword = () => {
  ...
  const onSubmitEmail = async(e) => {
    ...
    setLoading(true);
    try{
     const response =  await axios.post(backendURL+"/api/send-reset-otp?email="+email);
     if(response.status === 200){
      toast.success("Password reset otp sent succesfully.");
      setIsEmailSent(true);
     }else{
  ...
  const onSubmitNewPassword = async(e) => {
    ...
    setLoading(true);
    try{
      const response = await axios.post(backendURL+"/api/reset-password",{email,otp,newPassword});
      if(response.status === 200) {
          ...
          <form onSubmit={onSubmitNewPassword}>
            <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
              ...
              <input type="password" className='form-control bg-transparent border-0 ps-1 pe-4 rounded-end'
                placeholder='************' style={{ height: "50px" }} onChange={(e) => setNewPassword(e.target.value)}
